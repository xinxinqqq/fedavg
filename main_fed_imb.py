...
from Update import LocalUpdate,test_img,FedAvg,DatasetSplit
...
if __name__ == '__main__':
    ...
    TAG = 'exp/fed/{}_{}_{}_C{}_iid{}_{}_user{}_{}'.format(args.dataset, args.model, args.epochs, args.frac, args.iid,
                                                           args.imb_factor, args.num_users, current_time)
    ...
    validloader = DataLoader(DatasetSplit(train_dataset, idx_to_meta), batch_size=5, shuffle=True)

    test_loader = DataLoader(test_dataset, batch_size=args.test_bs, shuffle=False)
    # for i in range (len(user_groups)):
    #     print('****:',train_dataset.targets[user_groups[i]])
    ...
    model = ResNet32(args.dataset == 'cifar10' and 10 or 100).cuda(args.device)
